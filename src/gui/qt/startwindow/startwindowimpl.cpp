	...
	: myConfig(c), myLog(l), msgBoxOutdatedVersionActive(false)
{
	...
	// #endif
	setupUi(this);
	//this->setWindowTitle(QString(tr("TeamPoker %1").arg(POKERTH_BETA_RELEASE_STRING)));
	//this->setWindowTitle(QString(tr("TEAM POKER(R)")));
	
	QString msg(QString("TEAM POKER") + QChar(0xAE));
...
void startWindowImpl::networkError(int errorID, int /*osErrorID*/)
{
	...
	case ERR_SOCK_INVALID_PACKET: {
		MyMessageBox::warning(this, tr("Network Error"),
							  tr("An invalid network packet was received.\nPlease make sure that all players use the same version of TEAM POKER®."),
							  QMessageBox::Close);
	}
	break;
	...
	case ERR_NET_VERSION_NOT_SUPPORTED: {
		MyMessageBox msgBox(QMessageBox::Warning, tr("Network Error"),
							tr("The PokerTH server does not support this version of the game.<br>Please go to <a href=\"http://www.teampoker.com/\" target=\"_blank\">http://www.teampoker.com</a> and download the latest version."),
...
void startWindowImpl::networkNotification(int notificationId)
{
	...
	break;
	case NTF_NET_JOIN_NO_CONTACT_RULE: {
		MyMessageBox::warning(this, tr("Network Notification"),
							  tr("NO CONTACT RULE: You can't join this game since there is a clubmember already in it for the current event."),
							  QMessageBox::Close);
	}
	break;
	...
	case NTF_NET_NEW_RELEASE_AVAILABLE: {
		...
		msgBoxOutdatedVersion.setText(tr("A new release of TEAM POKER® is available.<br>Please go to <a href=\"http://www.teampoker.com/\" target=\"_blank\">http://www.teampoker.com</a> and download the latest version."));
	...
	case NTF_NET_OUTDATED_BETA: {
		...
		msgBoxOutdatedVersion.setText(tr("This beta release of TEAM POKER® is outdated.<br>Please go to <a href=\"http://www.teampoker.com/\" target=\"_blank\">http://www.teampoker.com</a> and download the latest version."));
		msgBoxOutdatedVersion.setTextFormat(Qt::RichText);
		msgBoxOutdatedVersion.setStandardButtons(QMessageBox::Ok);
		msgBoxOutdatedVersion.setDefaultButton(QMessageBox::Ok);
