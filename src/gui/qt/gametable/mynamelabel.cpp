				...
				if(myW->getSession()->getCurrentGame()) {
					...
					for (it_c=seatsList->begin(); it_c!=seatsList->end(); ++it_c) {
						if((*it_c)->getMyActiveStatus()) {
							++playerCounter;
							nickList += QString("&nick[]=");//.arg(playerCounter);
							nickList += QUrl::toPercentEncoding(QString::fromUtf8((*it_c)->getMyName().c_str()));
						}
					}
				}
														 
				linkString = QString("http://teampoker.com/game/platform/?fn=tableview&table="+QUrl::toPercentEncoding(QString::fromUtf8(info.name.c_str()))+nickList);
			} else {
				linkString = QString("http://teampoker.com/game/platform/?fn=tableview&nick="+QUrl::toPercentEncoding(t));
			}
