		...
		if(myConfig->readConfigString("LogDir") != "" && logDir.exists()) {
			...
			if(HTML_LOG) {
				...
				if(SQLITE_LOG) {
					...
					myHtmlLogFile_old = new QFile(myLogDir->absolutePath()+"/teampoker-log-"+currentTime.toString("yyyy-MM-dd_hh.mm.ss")+"_old.html");
				} else {
					//myHtmlLogFile_old = new QFile(myLogDir->absolutePath()+"/pokerth-log-"+currentTime.toString("yyyy-MM-dd_hh.mm.ss")+".html");
					myHtmlLogFile_old = new QFile(myLogDir->absolutePath()+"/teampoker-log-"+currentTime.toString("yyyy-MM-dd_hh.mm.ss")+".html");
				}
...
//                myW->textBrowser_Log->append(myHtmlLogFile_old->fileName());
				...
				stream_old << QString("<h3><b>Log-File for TEAMPOKER %1 Session started on ").arg(POKERTH_BETA_RELEASE_STRING)+QDate::currentDate().toString("yyyy-MM-dd")+" at "+QTime::currentTime().toString("hh:mm:ss")+"</b></h3>\n";
			...
			QStringList filters("teampoker-log*");
...
//                cout << logFileList.at(i).toStdString() << endl;
				...
				dateString.remove("teampoker-log-");
...
int guiLog::exportLog(QString fileStringPdb,int modus,int uniqueGameID_req)
{
	...
	if( mySqliteLogDb != 0 ) {
		...
		log_string += "Log-File for TEAMPOKER ";
		...
		if(!data_found) {
			cout << "Missing TEAMPOKER version information!" << endl;
			cleanUp(results, mySqliteLogDb);
			return 1;
		}
